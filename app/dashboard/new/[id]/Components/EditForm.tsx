...
const EditForm = ({ id, content, title, image }: Props) => {
  ...
  const [isUploading, setIsUploading] = useState(false); // Explicit image upload state
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="px-16 flex flex-col">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="mt-2">
              <FormControl>
                <Input
                  className="h-full text-4xl text-neutral-400 bg-transparent font-bold"
        ...
        <SubmitButton
          title="Save Changes"
          pending={isSubmitting || isUploading}
